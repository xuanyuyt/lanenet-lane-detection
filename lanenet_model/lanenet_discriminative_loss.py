...
def discriminative_loss_single(
    ...
    distance = tf.norm(tf.subtract(mu_expand, reshaped_pred), axis=1, ord=1)  # gb& ||mu_c - x_i||
    ...
    l_reg = tf.reduce_mean(tf.norm(mu, axis=1, ord=1))  # mean[sqrt(sum(mu_c^2))]
...
def discriminative_loss(prediction, correct_label, feature_dim,
    ...
    def body(label, batch, out_loss, out_var, out_dist, out_reg, i):
        disc_loss, l_var, l_dist, l_reg = discriminative_loss_single(
            prediction[i], correct_label[i], feature_dim, delta_v, delta_d, param_var, param_dist, param_reg)
    ...
    # 分别计算每个样本的 discriminative_loss， 随后 stack 到一起
